...
for line in f:
    ...
    myValue1 = atan2(-acc_x, acc_z)

    # # in order to show a plot use this function to append your value to a list:
    pitchData.append(myValue * 180.0 / pi)
    rollData.append(myValue1 * 180.0 / pi)
    ######################################################
...
# show the plot
if showPlot:
    plt.title("Roll and Pitch static")
    plt.ylabel("Angle")
    plt.xlabel("Time")
    plt.plot(pitchData, label="Pitch Data", color="b")
    plt.plot(rollData, label="Roll Data", color="g")
    plt.legend(loc="best")
    plt.savefig('imu_exercise_4.1.3_plot.png')
    plt.show()
